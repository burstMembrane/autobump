# cli.py
from pathlib import Path

import typer

from autobump.main import NoCommitsError, bump_version_from_git

app = typer.Typer()


@app.command()
def bump(
    project_file: Path = typer.Option("pyproject.toml", help="Path to pyproject.toml"),
    dry_run: bool = typer.Option(False, help="Show version change without writing"),
):
    """
    Bump version based on git commit history.
    """
    try:
        new_version = bump_version_from_git(project_file, dry_run=dry_run)
        typer.echo(f"✅ New version: {new_version}")
    except NoCommitsError:
        typer.echo("❌ No commits found in the repository.")


if __name__ == "__main__":
    app()
